...
namespace Harris.GPC
{
	...
	public class BaseGameManager : MonoBehaviour
	{
		...
		private BaseMouseInput _mouseInput;

		public BaseMouseInput MouseInput => _mouseInput;
        private BaseKeyboardInput _kbInput;
		...
		{
			_mouseInput = GetComponent<BaseMouseInput>();
            _kbInput = GetComponent<BaseKeyboardInput >();
        ...
        {
			Debug.Log("world mouse pos: " + Camera.main.ScreenToWorldPoint(Input.mousePosition));
            //return Camera.main.ScreenToWorldPoint(Input.mousePosition);

			var ray = Camera.main.ScreenPointToRay(Input.mousePosition);     
            RaycastHit rayHit;               
            
            if (Physics.Raycast(ray, out rayHit,Mathf.Infinity,_groundLayerMask))
            {
                return rayHit.point;
            }
            return Vector3.zero;
